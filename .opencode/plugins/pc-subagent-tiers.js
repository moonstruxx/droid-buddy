...
function agentColor(name) {
  ...
  // Quoted: YAML treats an unquoted " #RRGGBB" as a comment (space before #),
  // which parses color as null and fails opencode's config schema.
  return `"${hex}"`
}
...
export const PcSubagentTiers = async ({ directory }) => {
  ...
  function normalizeTemplate(templateContent, name) {
    ...
    if (name && shouldDeriveColor(currentColor)) {
      ...
      changed = true
    } else if (currentColor && /^#[0-9a-fA-F]{6}$/.test(currentColor.trim())) {
      // Repair a hex color left unquoted by an older version of this plugin:
      // YAML treats " #RRGGBB" (space then #) as a comment, so an unquoted
      // hex parses as color: null and fails opencode's config schema.
      fm = fm.replace(/^color:.*$/m, `color: "${currentColor.trim()}"`)
      changed = true
    }
    if (/^model:/m.test(fm)) {
