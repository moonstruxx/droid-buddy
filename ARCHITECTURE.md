...
<!-- Last updated: 2026-08-28 · circuit-processing-switch-detail: switch token + Value % rendering, processing pause (`p` key, `processing_paused` field, dimmed panels + `PROCESSING PAUSED` status, mutations blocked), per-circuit disable (`x` key, `disabled_circuits` + `hovered_graph_node` fields, `toggle_circuit_processing`/`rebuild_graph`/`toggle_processing_pause`, influence dead-ends at disabled sinks, `graph_node_dim`/`graph_edge_dim` tokens with error-red precedence, gallery scenarios) -->
