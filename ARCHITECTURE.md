...
│   ├── handler.rs          # keyboard + mouse event handling (incl. graph drag + label overlay `e`/`1..N`)
│   ├── patch.rs            # domain model (Patch, HwComponent, …) + .ini parser,
│   │                       #   virtual-cable index + banner-group extraction + preamble_labels
...
│   └── ui.rs               # ratatui rendering (panels, viewer, graph, status, picker + label overlay z-layer)
...
| toml | 0.9 | `config.toml` + `labels.toml` parse/serialize (`theme` + `[labels]` layers) |
...
- **config.toml**: user preferences file under `$XDG_CONFIG_HOME/droid-tui/` (schema: `theme` + `[labels] layers_enabled/max_shift_layer`).
...
<!-- Last updated: 2026-08-28 · label-management: [labels] config (layers_enabled/max_shift_layer 1..8) + per-patch labels.toml (hw per-shift + circuits per-NodeId), display_label fallback chain + overlay (e/1..N/Enter/Esc) with modifier_hue hint, panel/source/graph overrides -->
