...
│   │                       #   graph, handler, layout, patch, physical, schema, theme, ui,
│   │                       #   validation (+ regression in cfg(test))
...
- **Caching**: `load_schema() -> &'static Schema` returns the process-wide cached schema (filled lazily on first call from the embedded JSON and leaked; later calls return the same instance). The cache is a `Mutex<Option<&'static Schema>>`, not a `OnceLock`, so tests can reset it; `set_test_schema` / `reset_test_schema` (theme.rs idiom) let parallel tests pin or clear their view of the cache without cross-test poisoning. `merge_plugins` (plugin overlay) produces an owned `Schema`; task 2.2 sets the merged result here via `schema::init`.
