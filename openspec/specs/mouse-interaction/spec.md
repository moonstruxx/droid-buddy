...
The status bar SHALL compose each segment (Scale and any other status segments) exactly once per frame; no segment SHALL be duplicated regardless of the input method (keyboard or mouse) that triggered the state change.

#### Scenario: Scale shown once
- **WHEN** a patch is loaded and the scale state is changed
- **THEN** the status bar shows the Scale segment exactly once
