...
- [x] In `src/ui.rs` boxed rendering: render each control kind's state inside its joined box — knob/encoder percentage display, switch glyph, button state, fader — mirroring the LED-folded interior row; associated LEDs never render as standalone cells. Verify: insta snapshot / gallery scenario showing joined boxes for pot+LED, encoder+LED, switch+LED (fixture with such pairings); `cargo test` green. <!-- agent: layout-designer-engineer.build, depends_on: [2.1], touches: [src/ui.rs, src/regression.rs, fixtures/] -->

**details**: Acceptance: all control kinds with a resolvable LED association render as one boxed cell; associated LEDs never render standalone. Note: the boxed path in `render_component` was already kind-generic (per-kind `(symbol, state_text, fg_color)` triple from the 1.x/5.x work) — the deliverable here is the verification snapshot `visual_joined_boxes_kinds_snapshot` on `fixtures/led_pairs_kinds.ini` (all four kinds × numbered/bare pairing, all 8 LEDs folded, 3 themes).