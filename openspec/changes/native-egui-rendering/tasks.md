...
- [ ] 1.1 Promote the `gui` feature to unconditional, drop the `kitty-gfx` feature, and make winit/egui/egui-winit/wgpu required dependencies. Keep ratatui/crossterm/tiny-skia/fontdue/base64/flate2 in place until the teardown (3.1) removes them. Verify `cargo build` succeeds with egui and the terminal both present. <!-- agent: api-engineer.build, depends_on: [], touches: [Cargo.toml, src/lib.rs] -->
- [ ] 1.2 Split gui.rs into src/gui/mod.rs (window shell + surface dispatch) and move the graph canvas into src/gui/graph.rs. Verify `cargo build` passes and the window opens showing the existing graph canvas. <!-- agent: api-engineer.build, depends_on: [1.1], touches: [src/gui.rs, src/gui/mod.rs, src/gui/graph.rs] -->

## 2. Surface port to egui

- [ ] 2.1 Port the physical 1:1 view to an egui draw routine (mm grid, mm-to-pixel ScreenMapping, pan/zoom, skeleton toggle, element state cells) with pan/zoom/skeleton input. Verify with a shape/label test asserting element cells and pan/zoom math. <!-- agent: layout-designer-engineer.build, depends_on: [1.2], touches: [src/gui/physical.rs] -->
