...
- [ ] 2.2 Grid-model unit tests <!-- agent: horst-engineer.build, depends_on: [2.1], touches: [src/physical.rs] -->
      · chain order matches patch declaration; element cells resolve per token family; two `[p2b8]` instances yield two faceplates at real widths; unknown controller falls back to 5 HP; deterministic across runs
...
- [ ] 4.1 mm→chars mapping + pan/zoom model <!-- agent: layout-designer-engineer.build, depends_on: [2.1, 3.1], touches: [src/physical.rs, src/ui.rs, src/app.rs] -->
      · aspect-compensated mapping (D4: rows/mm ≈ 2 × cols/mm); `physical_offset` + `physical_zoom` state on App; scale presets (`+`/`-`) map to zoom levels around a fixed anchor; wheel-pan when the rack overflows; component_rects published from the same formula (renderer-owns-geometry contract)
      · verify: `cargo test` mapping unit tests (round-trip mm→screen→mm, zoom anchor stability, pan offset math)
- [ ] 4.2 Replace main panel view with physical render <!-- agent: layout-designer-engineer.build, depends_on: [4.1], touches: [src/ui.rs] -->
      · the main view renders components onto the grid cells (labels/state/shift/dim/pause, LED-folding + boxed cells preserved); multi-circuit instances render as side-by-side faceplates; over-long labels ellipsize; component_rects hit rects exactly match rendered cells
...
- [ ] 4.4 `[physical]` config defaults <!-- agent: rusty-engineer.fast, depends_on: [4.2], touches: [src/config.rs] -->
      · optional `[physical]` config (default zoom, pan/scroll behavior) with the existing injected-validation/atomic-write pattern
...
- [ ] 5.1 Coincidence proof tests <!-- agent: horst-engineer.build, depends_on: [3.3, 4.2], touches: [src/regression.rs, fixtures/**] -->
      · for each physical-layout fixture at fixed viewports: every full-view element rect equals its skeleton cell (same scale/offset); a fixture with two `[p2b8]` instances proves the faceplate path; overflow fixture proves pan consistency
...
- [ ] 5.3 Update ARCHITECTURE.md/DESIGN.md + sync specs <!-- agent: devops-engineer.fast, depends_on: [4.4, 5.2], touches: [ARCHITECTURE.md, DESIGN.md, openspec/specs/**] -->
      · document the physical grid model, skeleton mode, 1:1 main view, and verification harness; sync the delta specs (controller-panels, module-scaling, keybinding, mouse-interaction, visual-validation) into `openspec/specs/`
