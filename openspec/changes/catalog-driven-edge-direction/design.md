...
- [The nonstandard output parameter has no real embedded example] → the embedded circuits conventionally name their output `output`, so the "source from a nonstandard output parameter" scenario is exercised with a schema-level fixture (`set_test_schema` or a plugin circuit that declares a differently named output), not a hand-built embedded circuit.
