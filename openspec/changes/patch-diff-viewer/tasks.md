...
- [x] 1.2 Add `src/diff.rs#tests` + fixtures: reordered panels → equal; added/removed circuit; cable added; cable sink changed; changed param value; param reorder → equal; deterministic (order-independent) <!-- agent: horst-engineer.build, depends_on: [1.1], touches: [src/diff.rs#tests] -->
