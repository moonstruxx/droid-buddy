...
Every color in the interface is a named semantic token resolved from the active theme at render time; rendering code contains no raw color literals. The token set covers component kinds (`button`, `knob`, `cv_in`, `cv_out`, `led`), shift groups (`shift1`–`shift4`), chrome (`accent`, `muted`, `text`, `status_bg`), viewer keys/hints (`viewer_key`), viewer highlights (`focus_border`, `occurrence_highlight`, `modifier_boolean`, `modifier_exact`), the four minimap signal colors (`minimap_occurrence`, `minimap_modifier_boolean`, `minimap_modifier_exact`, `minimap_combined`), the tiled-layout pane borders (`pane_focus_border` — yellow classic, Reset terminal, white mono — and `pane_unfocused_border` — dark gray classic/mono, Reset terminal; the focused pane's border carries the focus token, all other panes the unfocused token), the graph surface tokens — node chrome (`graph_node_border`, `graph_node_title`), port markers (`graph_port_input`, `graph_port_output`), cluster chrome (`graph_cluster_border`, `graph_cluster_title`), the five cable-edge colors (`graph_edge_control`, `graph_edge_audio`, `graph_edge_midi`, `graph_edge_unknown`, `graph_edge_error`) — the latency ramp (`graph_edge_latency_0`–`_4` blue→red, with the mono variant a grayscale ramp and terminal all-Reset) plus `graph_edge_latency_legend` — the structural-diff cable colors (`graph_edge_diff_added` green, `graph_edge_diff_removed` magenta) — the validation severity tokens (`validation_error` red, `validation_warning` yellow, `validation_hint` cyan) plus modal chrome (`validation_modal_border`, `validation_selected_bg`), the render-outlier token (`render_outlier_warning` — classic yellow, mono white, terminal reset) for the predicted-degraded render status hint — the legacy optimizer menu chrome (`optimizer_modal_border` blue, `optimizer_selected_bg` dark gray; superseded by the pane focus border tokens — new code uses `pane_focus_border`, no `#[deprecated]` attribute) — the physical-skeleton tokens (`physical_skeleton_module_outline` — classic white, terminal reset, mono light gray; `physical_skeleton_cell` cyan; `physical_skeleton_port_in` cyan; `physical_skeleton_port_out` green; mono keeps outline/cell/ports pairwise distinct, terminal all-reset) — the fader LED-bar token (`fader_led_bar`, amber, distinct from the generic LED red) for the physical view's fader track fill — and the label overlay hint, which reuses `modifier_hue(hash%16)` (same hue family as the influence wash) with `graph_edge_error` red precedence kept.
...
- **Kitty-graphics image path**: `kitty-gfx` is a **default feature** (design D6), so when the terminal supports the kitty graphics protocol the surface renders as an anti-aliased image by default instead of box-drawing — rounded-rect nodes with a title bar, colored cable curves with direction arrows, rasterized text labels. The image is emitted (base64 + zlib, `f=32` RGBA, chunked ≤ 4096 bytes) and placed `z=-1` beneath the header/status/picker text so those cells stay readable on top; pan/zoom re-transmits with the same `i=` id (no id exhaustion). The pixel renderer derives every node/edge/label color from the active theme's semantic tokens via `Theme::rgb(Color)` (no hardcoded RGB) with the existing precedence (error red > diff > latency ramp > cable kind) unchanged. The offscreen rasterizer (`tiny-skia` rounded rects/Bézier strokes + `fontdue` rasterized labels) runs in `src/graph_render.rs`, the emitter in `src/kitty_protocol.rs`. On an unsupported terminal (or the `TestBackend`/snapshot path, or a build with `--no-default-features`) the surface falls back to the box-drawing renderer above with no error. All existing graph interactions — node drag, hover, `x` disable, `e` label overlay, diff/latency/topology-error coloring — are preserved on the image path; `graph_node_rects` are derived from the same camera the image uses (`GraphCamera`, `pixel = world × zoom − pan`) so pointer hit-testing stays aligned.
- **GPU graph window (gui feature)**: with the non-default `gui` feature built, `g w` opens the same graph in a winit/egui/wgpu desktop window (or `g g` opens the window instead of the tile when `[gui] graph_window = true`). The window paints the backend-neutral `SceneSpec` under the shared `GraphCamera` (one spec pixel = one egui point), so node positions, edges, and colors match the terminal tile; every interaction maps onto the same `App` mutations — drag → `local_resettle` + `NodeMoved`, hover, `x` disable, `p` pin, `e` label overlay. Canvas polish: middle-drag pan + cursor-anchored wheel zoom (deltas applied to the shared camera so both surfaces move together), marquee selection, a corner minimap with viewport indicator when the graph overflows, and a hover tooltip showing the circuit name + latency readout. Clicking a node sets `App.selected_circuit` (shared with the tile): the source viewer jumps to the section, the panels highlight the associated hardware, and the tile emphasizes the same node. All window colors — scene pixels and egui chrome alike — derive from theme tokens via `Theme::rgb`/`Theme::egui_color`, so switching the theme re-themes window and terminal together; no hardcoded RGB in the window. The window never opens under `cargo test`; without the feature `g w` shows a status hint.
