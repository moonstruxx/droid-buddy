...
/// A pending native-window action: queued at startup by
/// `main.rs::queue_startup_window_open` and consumed (with reset) by the
/// windowed run-loop on its next frame.
#[derive(Debug, Clone, Copy, PartialEq, Eq, Default)]
pub enum GraphWindowRequest {
    /// Nothing pending; the windowed loop only acts on a non-default value.
    #[default]
    None,
    /// Open the window if closed (idempotent).
    Open,
}
...
impl App {
    ...
    /// cannot reach `GraphWindow` (owned by the windowed loop in `main.rs`),
    /// so it records the request here and the loop opens the window on its
    /// next frame.
    pub fn request_graph_window(&mut self, request: GraphWindowRequest) {
        self.graph_window_request = request;
    }

    /// Poll-and-clear the pending native-window request. The windowed
    /// run-loop in `main.rs` calls this every frame and opens the window on
    /// `Open`. A failed open (headless display) leaves it closed; the app
    /// keeps running.
    ...
    pub fn rebuild_graph(&mut self) {
        ...
        self.emit_graph_built();
        // Re-apply influence highlights after rebuild so the graph tile stays
        // in sync with the current modifier selection.
        if let Some(sub) = self.influence.clone() {
    ...
    /// so the graph re-flows under the new spring stiffness; the status always
    /// reports the (possibly clamped) value. Determinism contract: same patch
    /// + same machine + same tension → same layout.
    ...
    /// When a non-empty root set exists the method applies highlights to the
    /// full graph and emits `InfluenceRecomputed`. With no selection or no
    /// derived vars the influence state is cleared.
    ...
    /// from the patch (`open_graph`), NOT on every `rebuild_graph`: an explicit user unpin (`p` on the tip) must survive
    ...
    /// Also recomputes modifier influence so graph highlight stays in sync.
